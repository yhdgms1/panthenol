 ...
 * and never expires. {@link net.minecraft.client.renderer.texture.HttpTexture} also keeps
 * a disk file per hash and will not re-fetch while that file exists. TLauncher-style APIs
 * often keep a stable URL when the PNG bytes change, so clearing only Panthenol's JS memo
 * is not enough — the hash must be marked here so the next {@code TextureCache#getOrLoad}
 * drops the in-memory entry and disk file before registering again.
 */
public final class SkinReload {
    ...
    public static void markTextures(@Nullable MinecraftProfileTextures textures) {
    ...
    public static void markTexture(@Nullable MinecraftProfileTexture texture) {
        if (texture == null) {
            return;
        }
